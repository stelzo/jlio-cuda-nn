...
private:
    thrust::device_ptr<thrust::device_vector<PhcCube>> phc_cubes_;
...
class IVox {
   public:
    ...
    explicit IVox(Options options) : options_(options) {
        ...
        Value constexpr empty_value_sentinel = 0;
   ...
   private:
    ...
    Options options_;
    std::unordered_map<KeyType, thrust::pair<KeyType, IVoxNodePhc>*, hash_vec> grids_map_un;

    std::unique_ptr<cuco::dynamic_map<MapKeyT, MapValueT>> grids_map_; // voxel hash map. initial size must be larger (cloud_n / voxel_size) size minimum
    stdgpu::deque<thrust::pair<KeyType, IVoxNodePhc>> grids_cache_;  // voxel cache
